...
n_ascan=500; %number of Ascans - here M-mode is considered
...
gauss_envelope = gauss_distribution(x,n_samples_per_sweep/8, n_samples_per_sweep/2);
...
for ii=1:n_ascan
    ...
    %OCT_fringe(ii,:)=surface_reflectivity*((cos(2*Delta(ii)*k_vector)+1).*gauss_envelope)+noise_array;
    OCT_fringe(ii,:)=surface_reflectivity*((cos(2*Delta2(:,ii)'.*k_vector)+1).*gauss_envelope).*noise_array;
